...
/// Previous state snapshot used to detect changes and avoid redundant UI updates.
...
/// Start a 5-second periodic poller that updates the summary row, connectivity,
/// and WiFi radio state. Only triggers UI updates when something actually changes.
pub fn start_periodic_poller(state: Rc<RefCell<NetworkState>>, w: PollerWidgets) {
    ...
    glib::timeout_add_local(std::time::Duration::from_secs(5), move || {
        ...
        if radio_changed {
            ...
            wifi_switch_set_state_no_signal(&w.wifi_switch, wifi_radio);
...
/// Set a Switch's visual state without triggering `state-set`.
fn wifi_switch_set_state_no_signal(sw: &gtk4::Switch, active: bool) {
    sw.set_state(active);
}
...
pub fn update_connectivity_display(
    ...
    if let Some(badge) = connectivity.summary_badge() {
